...
| **POST** | `/agents/:agentId/run` | Body: **`message`** (required), optional **`sessionId`**, **`expiresAtMs`**, **`extendSessionTtlMs`**, **`projectId`** (multi-tenant), **`wait`** (with **`dispatch`**). Inline: **`Agent.run`**. JSON responses include effective **`projectId`**. Queue: **`engine.addRun`** → **202** + **`jobId`** / **`projectId`** / **`statusUrl`**, or **`?wait=1`** / **`wait: true`** with **`queueEvents`**. |
| **POST** | `/agents/:agentId/resume` | Body: **`runId`**, **`sessionId`**, **`resumeInput` `{ type, content }`**, optional **`expiresAtMs`**, **`extendSessionTtlMs`**, **`projectId`**, **`wait`**. Inline needs **`runStore`** on **`AgentRuntime`**. JSON responses include effective **`projectId`** when enqueuing or completing inline. |
...
| **GET** | `/agents/:id/runs` | List run summaries (dashboard) | ✓ (**`RunStore.listByAgentAndSession`** when **`sessionId`** is supplied, else **`listByAgent`**; tenant filter when **`run.projectId`** set — debt §1) |
