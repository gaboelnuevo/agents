...
| **Allowlist + session + RAG + vector + send_message + tool failures** | **`runtime-tool-allowlist.test.ts`**, **`session-expiry.test.ts`**, **`rag-file-catalog.test.ts`**, **`vector-limits.test.ts`**, **`send-message-validation.test.ts`**, **`tool-failure-observation.test.ts`**, **`run-store.test.ts`** — see **`plan.md`** Phase **9** steps **9.7–9.11** and Progress snapshot. |
