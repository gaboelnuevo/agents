...
export function chatAgentToolIds(config: ResolvedRuntimeStackConfig): string[] {
  if (!config.artifacts.enabled) {
    return [...DEFAULT_CHAT_AGENT_TOOL_IDS];
  }
  return [
...
- **Artifacts:** When the planner generates files (reports, JSON, images), they are stored as artifacts. Use **system_list_artifacts** to list all artifacts for the current session, and **system_write_artifact** to create new artifacts if needed.
